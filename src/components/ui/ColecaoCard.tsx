    ...
    <div className="group relative flex flex-col bg-surface-900/90 dark:bg-surface-900 border border-teal-500/20 hover:border-teal-500/50 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-teal-500/10 transition-all duration-300 hover:-translate-y-1.5">
      ...
      <div className="relative h-48 w-full overflow-hidden bg-surface-950">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2">
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-surface-950/80 backdrop-blur-md text-teal-300 border border-teal-500/30 shadow-md">
            {colecao.numeroTombo}
          </span>
        ...
        {/* Exemplares Pill */}
        <div className="absolute bottom-3 left-3 flex items-center gap-1 bg-teal-950/80 backdrop-blur-md text-teal-300 px-2.5 py-1 rounded-lg border border-teal-500/30 text-xs font-semibold">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4 text-surface-100">
        <div>
          ...
          {taxoList ? (
            <div className="mt-1.5 flex items-center gap-1.5 text-xs text-teal-400 font-medium truncate">
          ...
          <div className="mt-3 space-y-1.5 text-xs text-surface-300">
            {colecao.localidade && (
            ...
            <div className="flex items-center justify-between text-surface-400 pt-1">
              <span>Frasco: <strong className="text-surface-200">{colecao.condicaoFrasco || "RAZOAVEL"}</strong></span>
              {colecao.determinador && (
                <span>Det: <strong className="text-surface-200">{colecao.determinador}</strong></span>
              )}
            </div>
          </div>
        </div>
