...
export function TeamCard({ users, currentUser, onUserRoleChange }: TeamCardProps) {
    ...
    switch (role) {
      case "admin":
        return {
          ...
          badgeClass: "bg-purple-100 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 border-purple-500/30",
        };
      case "monitor":
        return {
          label: "Monitor(a)",
          ...
          badgeClass: "bg-teal-100 dark:bg-teal-950/80 text-teal-700 dark:text-teal-300 border-teal-500/30",
        };
      case "pesquisador":
      default:
        return {
          ...
          badgeClass: "bg-amber-100 dark:bg-amber-950/80 text-amber-700 dark:text-amber-300 border-amber-500/30",
        };
    }
  };

  // Convert displayUsers into CarouselItem array for KlarnaCarousel
  ...
  return (
    <Card className="animate-slide-up bg-surface-900 text-white border border-teal-500/30 shadow-2xl p-4 sm:p-6 lg:p-8 overflow-hidden relative">
      {/* Background ambient lighting */}
      <div className="absolute -right-24 -top-24 w-96 h-96 bg-teal-500/10 rounded-full filter blur-3xl pointer-events-none" />
      <div className="absolute -left-24 -bottom-24 w-96 h-96 bg-purple-500/10 rounded-full filter blur-3xl pointer-events-none" />

      {/* Header */}
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-surface-800">
        <div>
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-teal-500/20 text-teal-400 border border-teal-500/30 shadow-lg shadow-teal-500/10">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-teal-500/20 text-teal-300 border border-teal-500/30">
        ...
        {isAdmin && (
          <span className="self-start sm:self-center inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl bg-purple-500/20 text-purple-300 border border-purple-500/40 shadow-xs">
      ...
      <div className="relative z-10 my-4 py-2 bg-gradient-to-b from-surface-950/80 to-surface-900/90 rounded-3xl border border-surface-800 shadow-inner overflow-hidden">
        <KlarnaCarousel
          items={carouselItems}
          imageWidth={260}
          imageHeight={260}
      ...
      {activeUser && (
        <div className="relative z-10 mt-6 p-4 sm:p-5 rounded-2xl bg-surface-800/80 border border-teal-500/30 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0 w-full md:w-auto">
            <img
              src={(activeUser as any).imagemUrl || AVATAR_POOL[activeIndex % AVATAR_POOL.length]}
              alt={activeUser.name}
              className="w-14 h-14 rounded-2xl object-cover border-2 border-teal-400/50 shadow-md shrink-0"
            />
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {currentUser?.id === activeUser.id && (
                  <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-md bg-teal-500 text-surface-950 shadow-xs">
                    Você
                  </span>
                )}
              </div>

              <div className="flex items-center gap-3 text-xs text-surface-400 mt-1 flex-wrap">
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-5 border-t border-surface-800">
        <div className="p-3.5 rounded-2xl bg-purple-950/40 border border-purple-500/30 text-xs">
          <div className="flex items-center gap-1.5 mb-1 text-purple-300 font-extrabold">
        ...
        <div className="p-3.5 rounded-2xl bg-teal-950/40 border border-teal-500/30 text-xs">
          <div className="flex items-center gap-1.5 mb-1 text-teal-300 font-extrabold">
            <Eye className="h-4 w-4 shrink-0 text-teal-400" />
            <span>Monitor(a)</span>
          </div>
          <p className="text-teal-200/70 leading-snug">
        ...
        <div className="p-3.5 rounded-2xl bg-amber-950/40 border border-amber-500/30 text-xs">
          <div className="flex items-center gap-1.5 mb-1 text-amber-300 font-extrabold">
            <Microscope className="h-4 w-4 shrink-0 text-amber-400" />
...
function RoleBadgeDisplay({ role }: { role: UserRole }) {
  switch (role) {
    case "admin":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-extrabold bg-purple-500/20 text-purple-300 border border-purple-500/40 shadow-xs">
          <Shield className="h-4 w-4" />
          Administrador
        </span>
      );
    case "monitor":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-extrabold bg-teal-500/20 text-teal-300 border border-teal-500/40 shadow-xs">
    ...
    default:
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-extrabold bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-xs">
  ...
  return (
    <div className="relative w-full md:w-auto">
      <button
        ...
        className={`w-full md:w-auto flex items-center justify-between gap-2.5 px-3.5 py-2 rounded-xl border text-xs font-extrabold transition-all shadow-md active:scale-95 ${currentConfig.color}`}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-72 p-2 bg-surface-900 rounded-2xl border border-teal-500/30 shadow-2xl z-50 animate-slide-up space-y-1">
            <p className="text-[10px] uppercase font-black text-surface-400 px-2 py-1 tracking-wider">
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-start gap-3 p-2.5 rounded-xl text-left transition-all ${
                    isSelected
                      ? "bg-teal-500/20 text-teal-200 font-extrabold border border-teal-500/40 shadow-xs"
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-surface-400 font-medium leading-snug">
                      {r.desc}
                    </div>
                  </div>
                </button>
